        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <!-- Search Field for Modal Table -->
                    <div class="search-wrapper-container my-3 text-center">
                        <input type="text" id="search-students-modal" class="search-input text-center"  placeholder="Search Date, Time, Status">
                    </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        // --- Table Search: Student Modal
        $('#search-students-modal').on('input', function () {
            const query = $(this).val().trim().toLowerCase();
            $('#reportDetailsBody tr').each(function () {
                const date = $(this).find('td:nth-child(5)').text().toLowerCase();
                const time = $(this).find('td:nth-child(6)').text().toLowerCase();
                const status = $(this).find('td:nth-child(8)').text().toLowerCase();
                $(this).toggle(date.includes(query) || time.includes(query) || status.includes(query));
            });
        });
    });
    ...
    let currentStudentCounts = {};

    // --- Update Class Attendance Charts (Pie + Bar)
    ...
    // --- Update Student Attendance Charts (Pie + Bar)
    ...
    // --- Handle Form Submission for Attendance Report
    ...
    $('#studentReportDownload').on('click', function () {
        ...
        const query = `?student_id=${encodeURIComponent(student_id)}&course_name=${encodeURIComponent(course_name)}&section=${encodeURIComponent(section)}`;
        window.location.href = `../Teacher/scripts/download_student_report.php${query}`;
    });

    // --- Handle View Student Report Modal
    let StudentReport = {};
